...
impl Tool for MakeDirTool {
    ...
    fn description(&self) -> &str {
        "Create a directory, including all parent directories. Path must be within the data directory."
    }
    ...
    fn parameters(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            "properties": {
                "path": {
                    "type": "string",
                    "description": "Path to the directory to create (within data directory)"
                }
            },
            "required": ["path"]
        })
