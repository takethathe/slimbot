...
async fn test_add_and_get_messages() {
    ...
    sm.get_or_create("test:chat1").await.unwrap();
    sm.add_message("test:chat1", Message::User { content: "hello".to_string() }).await.unwrap();
    sm.add_message("test:chat1", Message::Assistant { content: Some("hi".to_string()), tool_calls: None }).await.unwrap();
...
async fn test_add_message_unknown_session_fails() {
    ...
    let result = sm.add_message("nonexistent", Message::User { content: "x".to_string() }).await;
...
async fn test_persist_creates_jsonl_file() {
    ...
    sm.get_or_create("test:chat1").await.unwrap();
    sm.add_message("test:chat1", Message::User { content: "persist me".to_string() }).await.unwrap();
...
async fn test_reload_from_jsonl() {
    ...
    // First session: add messages and persist
    {
        let mut sm = SessionManager::new(session_dir.clone()).unwrap();
        sm.get_or_create("test:chat1").await.unwrap();
        sm.add_message("test:chat1", Message::User { content: "saved".to_string() }).await.unwrap();
        sm.persist("test:chat1").await.unwrap();
    }
...
async fn test_consolidation_cursor_skips_messages() {
    ...
    // Set cursor to skip first 2 messages
...
async fn test_consolidation_persists_and_reloads_cursor() {
...

    {
        ...
        sm.get_or_create("s1").await.unwrap();
        sm.add_message("s1", Message::User { content: "a".to_string() }).await.unwrap();
        sm.add_message("s1", Message::User { content: "b".to_string() }).await.unwrap();
...
async fn test_shared_session_manager_concurrent_access() {
    ...
    let h1 = tokio::spawn(async move {
        ...
        guard.add_message("shared:s1", Message::User { content: "from task1".to_string() }).await.unwrap();
    });

    let h2 = tokio::spawn(async move {
        tokio::time::sleep(tokio::time::Duration::from_millis(10)).await;
        let mut guard = sm2.lock().await;
        guard.add_message("shared:s1", Message::User { content: "from task2".to_string() }).await.unwrap();
    });
...
fn test_message_assistant_with_tool_calls() {
    ...
    let msg = Message::Assistant {
        content: Some("running ls".to_string()),
